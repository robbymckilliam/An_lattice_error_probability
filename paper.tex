...
A \term{lattice}\index{lattice}, $\Lambda$, is a discrete set of points in $\reals^m$ such that
\[
...
\end{align}
The probability of error is $P_E \leq 1 - P_C(\Lambda)$, where the upper bound is asymptotically tight for large constellations (the proportion of the codewords near the boundary of the shaping region becomes small).  By expanding $e^x = 1  + x + \frac{x^2}{2} + \dots$ we obtain
\begin{align}
