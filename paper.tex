...
We would like to be able to integrate functions over the Voronoi cell of $A_n$.  The definition of $A_n$ above places it in the $n$ dimensional zero-sum plane, lying in $\reals^{n+1}$.  The Voronoi cell is a subset of the zero-sum plane and has zero $n+1$-dimensional volume.  By an appropriate change of variables it would be possible to write the Voronoi cell in an $n$-dimensional coordinate system, and then integrate.  However, we find the following approach simpler.  Given a set $S$ let $S \times \onebf$ denote the set of elements that can be writen as $x + y$ where $x \in \vor(A_n)$ and $y = k\onebf$ for some $k \in [-\nicefrac{1}{2},\nicefrac{1}{2}]$.  Consider a function $f : \reals^{n+1} \mapsto \reals$.  We would like to be able to solve integrals of the form
