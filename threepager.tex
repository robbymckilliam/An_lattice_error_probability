...
%\hat{\cbf} &=& \mbox{arg~}\min_{\cbf \in \calC} \| \ybf - \cbf \|^2.
\end{equation}
That is, the receiver computes the lattice point in $\Lambda$ nearest in Euclidean distance to the received signal $\ybf$.  Equivalently, $\hat{\cbf}$ is the lattice point such that $\ybf \in \vor(\Lambda) + \hat{\cbf}$.  The codeword is correctly decoded when $\hat{\cbf} = \cbf$, or equivalently when $\ybf \in \vor(\Lambda) + \cbf$ implying that $\wbf \in \vor(\Lambda)$.  Assuming that each codeword $\cbf$ is transmitted with probability $p(\cbf)$, then the probability of correct decoding is
\begin{align}
...
\begin{figure*}[htbp]
	\centering
		\includegraphics{plots/threepagerpeplot-1.mps}
