...
The highly symmetric structure of these lattices provides the grounds for extremely efficient encoding and decoding algorithms.  In this paper we derive formulae for accurately predicting the performance of lattice codes constructed from $A_n$.  This is achieved by deriving formulae for the \emph{moments} of the \emph{Voronoi cell} of $A_n$.  %Conway and Sloane suggested this approach to compute the quantizing constants (second order moments) of the root lattices~\cite{Conway1982VoronoiRegions}.  Here, we extend their technique to compute the moments of any order for $A_n$.
...
%\end{figure}
\newcommand{\calM}{M}
Let $\calM_n(m) = \int_{\vor(A_n)} \|\xbf\|^{2m} d\xbf$ be a moment of the Voronoi cell of $A_n$.  Our main results is as follows.  The moment $\calM_n(m)$ satisfies
